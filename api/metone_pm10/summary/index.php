...
// Build the SQL query
$query = "SELECT DATE(time), round(cast(avg(value) as numeric),3) as average, round(cast(max(value) as numeric),3) as max, round(cast(min(value) as numeric),3) as min FROM metone WHERE unit_id = $1 AND season = $2 AND type = 'pm10' GROUP BY DATE(time) ORDER BY DATE(time)";
