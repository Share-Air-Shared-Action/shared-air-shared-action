...
// Build the SQL query
$query = 'SELECT DATE(created_at), round(cast(avg(pm1_cf_atm_ugm3) as numeric),3) as average, round(cast(max(pm1_cf_atm_ugm3) as numeric),3) as max, round(cast(min(pm1_cf_atm_ugm3) as numeric),3) as min FROM purpleairprimary WHERE device_name = $1 AND season = $2 GROUP BY DATE(created_at) ORDER BY DATE(created_at)';
