...
// Build the SQL query
$query = 'SELECT DATE(created_at), avg(pm1_cf_atm_ugm3) as average, max(pm1_cf_atm_ugm3) as max, min(pm1_cf_atm_ugm3) as min FROM purpleairprimary WHERE device_name = $1 AND season = $2 GROUP BY DATE(created_at) ORDER BY DATE(created_at)';
