...
// Build the SQL query
$query = 'select location.community as x, coalesce(purpleair.y,0) as y from (select community from stationarylocations group by community) location left join (select purpleair.community AS x, ROUND(CAST(AVG(pm1_cf_atm_ugm3) as NUMERIC),3) AS y FROM purpleair where error IS distinct FROM 1 and season=$1 group by purpleair.community) purpleair on location.community=purpleair.x';
