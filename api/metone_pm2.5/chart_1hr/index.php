...
// Build the SQL query
// $query = "SELECT time AS x, value_avg AS y FROM metone_1hr WHERE unit_id = $1 AND season = $2 AND community = $3 AND type = 'pm25' ORDER BY time";
$query = "select metone_1hr.time as x,round(metone_1hr.avg_value::numeric,3) as y1,(case when epa_hourly_pm25.sample_measurement=0 then -1 else epa_hourly_pm25.sample_measurement end) as y2 from metone_1hr,epa_sensors_community_lookup,epa_hourly_pm25 where metone_1hr.unit_id = $1 AND metone_1hr.season = $2 AND metone_1hr.community = $3 AND metone_1hr.type='pm25' AND epa_sensors_community_lookup.pollutant='pm2.5' AND metone_1hr.community=epa_sensors_community_lookup.community and epa_sensors_community_lookup.longitude=epa_hourly_pm25.longitude and epa_sensors_community_lookup.latitude=epa_hourly_pm25.latitude and to_char(metone_1hr.time, 'YYYY-MM-DD HH24:MI:SS')=to_char(epa_hourly_pm25.date_local+epa_hourly_pm25.time_local, 'YYYY-MM-DD HH24:MI:SS') order by 1";
