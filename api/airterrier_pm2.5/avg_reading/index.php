...
(select season,to_char(time,'HH24')::integer tod, measured_value from airterrier where community=$2 and (sensor_name=$3 OR sensor_name=$4) and (season='Summer' or season='Winter') and flag is null AND measurement_type = $1 ) part_1) part_2 group by season,day_section";

// Run the query
$result = pg_query_params($dbconn, $query, array($measurement_type,$community,$sensor_name, $sensor_name_comm)) or die (return_error("Query failed.", pg_last_error()));
