...
// Build the SQL query
// $query = "SELECT time AS x, value_avg AS y FROM aeroqualno2_1hr WHERE unit_id = $1 AND season = $2 AND community = $3 AND type = 'no2' ORDER BY time";
$query = "select aeroqualno2_1hr.date as x,round(aeroqualno2_1hr.no2ppm_avg::numeric*1000,3) as y1,(case when epa_hourly_no2.sample_measurement=0 then -1 else epa_hourly_no2.sample_measurement end) as y2 from aeroqualno2_1hr,epa_sensors_community_lookup,epa_hourly_no2 where aeroqualno2_1hr.unit_id = $1 AND aeroqualno2_1hr.season = $2 AND epa_sensors_community_lookup.pollutant='no2' AND aeroqualno2_1hr.community = $3 AND aeroqualno2_1hr.community=epa_sensors_community_lookup.community and epa_sensors_community_lookup.longitude=epa_hourly_no2.longitude and epa_sensors_community_lookup.latitude=epa_hourly_no2.latitude and to_char(aeroqualno2_1hr.date, 'YYYY-MM-DD HH24')=to_char(epa_hourly_no2.date_local+epa_hourly_no2.time_local, 'YYYY-MM-DD HH24') order by 1";
