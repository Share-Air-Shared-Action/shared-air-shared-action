...
// Build the SQL query
// Agregates from purpleairprimary and wundergound indvidual for each date and then joined
$query = "SELECT DATE(purpleair.date_val),ROUND(cast(purpleair.average as NUMERIC),3) average,ROUND(cast(purpleair.maximum as NUMERIC),3) AS max,ROUND(cast(purpleair.minimum as NUMERIC),3) as MIN,weather.temperature,weather.dewpoint,weather.pressure,weather.windspeed,weather.precipitation FROM (SELECT DATE(created_at) date_val,AVG(pm25_cf_atm_ugm3) AS average,max(pm25_cf_atm_ugm3) AS maximum,min(pm25_cf_atm_ugm3) AS minimum FROM purpleairprimary WHERE device_name=$1 AND community=$3 AND season=$2 AND error IS DISTINCT FROM 1 GROUP BY DATE(created_at)) AS purpleair LEFT JOIN (SELECT DATE(observation_time) date_val,ROUND(AVG(CAST(TEMP_F as NUMERIC)),3) AS Temperature, ROUND(AVG(cast(dewpoint_f as NUMERIC)),3) AS DewPoint, ROUND(AVG(CAST(pressure_in AS NUMERIC)),3) AS Pressure, ROUND(AVG(CAST(wind_mph AS NUMERIC)),3) AS WindSpeed, ROUND(SUM(CAST(case when precip_1hr_in < 0 then 0 else precip_1hr_in end AS NUMERIC)),2) AS Precipitation FROM wundergound WHERE community = $3 GROUP BY DATE(observation_time)) AS weather ON DATE(purpleair.date_val)=DATE(weather.date_val) ORDER BY purpleair.date_val";
