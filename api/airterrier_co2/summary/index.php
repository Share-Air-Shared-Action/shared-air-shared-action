...
// Build the SQL query
$query = "SELECT DATE(time), avg(measured_value) as average, max(measured_value) as max, min(measured_value) as min FROM airterrier WHERE measurement_type = 'CO2 concentration' AND session_title = $1 AND season = $2 GROUP BY DATE(time) ORDER BY DATE(time)";
