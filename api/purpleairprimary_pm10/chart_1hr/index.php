...
// Build the SQL query
// $query = "SELECT time AS x, value_avg AS y FROM purpleair_1hr WHERE unit_id = $1 AND season = $2 AND community = $3 AND type = 'pm10' ORDER BY time";
$query = "select hourly.created_at as x,y1,y2 from (select created_at,round(pm10_cf_atm_ugm3_avg::numeric,3) as y1 from purpleair_1hr where purpleair_1hr.device_name = $1 AND purpleair_1hr.season = $2 AND purpleair_1hr.community = $3) hourly left join (select epa_hourly_pm10.date_local+epa_hourly_pm10.time_local date_time, (case when epa_hourly_pm10.sample_measurement=0 then -1 else epa_hourly_pm10.sample_measurement end) as y2 from epa_sensors_community_lookup,epa_hourly_pm10 where epa_sensors_community_lookup.longitude::numeric=epa_hourly_pm10.longitude::numeric and epa_sensors_community_lookup.latitude::numeric=epa_hourly_pm10.latitude::numeric and epa_sensors_community_lookup.community= $3 AND epa_sensors_community_lookup.pollutant='pm10') epa on to_char(hourly.created_at, 'YYYY-MM-DD HH24:MI:SS')=to_char(epa.date_time, 'YYYY-MM-DD HH24:MI:SS') order by 1";
