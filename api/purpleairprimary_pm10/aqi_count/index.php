...
// Build the SQL query
$query = 'select community, (sum(case when pm10_cf_atm_ugm3 < 54 then 1 else 0 end)*100)/count(*) as good, (sum(case when pm10_cf_atm_ugm3 > 54 and pm10_cf_atm_ugm3 < 154 then 1 else 0 end)*100)/count(*) as moderate, (sum(case when pm10_cf_atm_ugm3 > 154 and pm10_cf_atm_ugm3 < 254 then 1 else 0 end)*100)/count(*) as unhfsg, (sum(case when pm10_cf_atm_ugm3 > 254 and pm10_cf_atm_ugm3 < 354 then 1 else 0 end)*100)/count(*) as unhealthy, (sum(case when pm10_cf_atm_ugm3 > 354 and pm10_cf_atm_ugm3 < 424 then 1 else 0 end)*100)/count(*) as very_unhealthy, (sum(case when pm10_cf_atm_ugm3 > 250.4 then 1 else 0 end)*100)/count(*) as hazardous from purpleair where community=$1 and error is distinct from 1 group by community';
