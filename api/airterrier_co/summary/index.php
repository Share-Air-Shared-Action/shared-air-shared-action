...
// Build the SQL query
$query = "SELECT DATE(time), round(cast(avg(measured_value) as numeric),3) as average, round(cast(max(measured_value) as numeric),3) as max, round(cast(min(measured_value) as numeric),3) as min FROM airterrier WHERE measurement_type = 'CO concentration' AND session_title = $1 AND season = $2 GROUP BY DATE(time) ORDER BY DATE(time)";
