...
// Build the SQL query
$query = 'select upper(substr(session_title,0,3)) as community, (sum(case when measured_value < 54 then 1 else 0 end)*100)/count(*) as good, (sum(case when measured_value > 54 and measured_value < 154 then 1 else 0 end)*100)/count(*) as moderate, (sum(case when measured_value > 154 and measured_value < 254 then 1 else 0 end)*100)/count(*) as unhfsg, (sum(case when measured_value > 254 and measured_value < 354 then 1 else 0 end)*100)/count(*) as unhealthy, (sum(case when measured_value > 354 and measured_value < 424 then 1 else 0 end)*100)/count(*) as very_unhealthy, (sum(case when measured_value > 424 then 1 else 0 end)*100)/count(*) as hazardous from airterrier where upper(substr(session_title,0,3))=$1 and error is distinct from 1 and measurement_type = $2 and sensor_name=$3 group by upper(substr(session_title,0,3))';
