...
(select season,to_char(time,'HH24')::integer tod, measured_value from airterrier where upper(substr(session_title,0,3))=$2 and AND sensor_name=$3 AND season='Summer' or season='Winter' and error IS DISTINCT FROM 1 AND measurement_type = $1 ) part_1) part_2 group by season,day_section";
